...
- **Environments:** Предусмотрены окружения `Dev` и `Prod` с независимыми
  ...
  окружение задаётся переменной `BOT_ENV` (`dev` по умолчанию). При значении
  `prod` используется токен `TELEGRAM_BOT_TOKEN_PROD`, во всех остальных случаях —
  `TELEGRAM_BOT_TOKEN_DEV`. Функция `dispatcher_from_env` автоматически выбирает
  корректный токен, читает `TELEGRAM_CHAT_ID` (поддерживается формат
  `123,456,789`) и создаёт `TelegramDispatcher` на основе переменных окружения.
