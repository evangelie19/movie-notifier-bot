...
- **Структура пайплайна:**
  0. Шаг `initialize_clients` — загрузка секретов `TMDB_API_KEY`,
     `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID` из переменных окружения
     (они заполняются из environment secrets `BOT_TOKEN` и `CHAT_ID`
     GitHub Actions в зависимости от выбранного окружения) и создание реальных
     клиентов `TmdbClient` (на базе `reqwest` с TLS) и `TelegramDispatcher` для
     работы с prod-ботом.
  ...
  6. Шаг `persist_history` — дополнение списка отправленных идентификаторов
     ...
     историю. Путь к файлу и имя артефакта можно переопределить переменными
     окружения `HISTORY_FILE_PATH` и `HISTORY_ARTIFACT_NAME` (удобно при
     разделении истории между dev/prod в отдельных шагах workflow). Файл
     представляет собой набор TMDB ID, разделённых переводами строк.
...
- **Текущий режим:** пока активен только Dev-стек. Все запуски (локально и в CI)
  используют environment secrets окружения `Dev` с именами `BOT_TOKEN` и
  `CHAT_ID`. В рантайме они мапятся в переменные `TELEGRAM_BOT_TOKEN` и
  `TELEGRAM_CHAT_ID` с Dev-значениями, чтобы исключить путаницу с конфигурацией.
- **Выход в прод:** Prod-окружение будет задействовано позже. Доставка в
  ...
  запуска. Секреты Prod-окружения именуются так же (`BOT_TOKEN`, `CHAT_ID`) и
  автоматически прокидываются в переменные `TELEGRAM_BOT_TOKEN` и
  `TELEGRAM_CHAT_ID` при выборе окружения `Prod` в workflow.
...
- **Финальное уведомление:** Job настроена с `continue-on-error: false`; при
  ...
  отправляет короткое сообщение в Dev-чат через токен из Dev environment secret
  `BOT_TOKEN` (прокидывается в переменную `TELEGRAM_BOT_TOKEN`).
...
- **Ручная доставка в Dev:** Workflow `.github/workflows/dev-delivery.yml`
  позволяет вручную прогнать доставку только в dev-чат с отдельной историей
  артефактов (можно задать `artifact_scope` и `rust_log` при запуске). Под
  капотом вызывает общий пайплайн `common-delivery` с параметрами
  `send_dev=true`, `send_main=false` и окружением `Dev`.
- **Продовая доставка с защитой:** Workflow
  `.github/workflows/prod-delivery.yml` сначала выполняет шаг доставки в dev
  (job `dry_run_in_dev`), а при его успехе запускает продовую отправку
  (job `deliver_to_prod`). Оба шага используют общий reusable workflow
  `common-delivery`, поэтому история отправок разделяется по суффиксам `dev`
  и `prod` и при необходимости может быть переопределена через
  `artifact_scope`.
...
- **Окружения:** Всегда есть два набора environment secrets (`BOT_TOKEN`,
  `CHAT_ID`, `TMDB_API_KEY`) — отдельные значения для окружений `Dev` и `Prod`.
  В разработке используется только Dev-комплект, чтобы все отправки шли в
  DevChat; Prod-комплект будет задействован позже и только после успешной
